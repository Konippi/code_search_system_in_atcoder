...
def create_db(cur, contest, problem):
    ...
    cur.execute(
        "CREATE TABLE atcoder_"
        + contest
        + "_"
        + problem
        + "(id INTEGER, user STRING,\
...
def set_db(cur, contest, problem):
    if (42 <= int(contest) and int(contest) <= 111) and (
        problem == "c" or problem == "d"
    ):
    ...
    for roop in range(int(PEOPLE_NUM / 20)):
        ...
        for j in i.find_all(
            href=re.compile("/contests/abc" + contest + "/submissions/")
        ):
